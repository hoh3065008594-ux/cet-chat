      ...
      <div className="flex flex-col h-full">
        <div className="flex-1 flex flex-col items-center justify-center text-gray-400 space-y-4">
          ...
          {(() => {
            const settings = JSON.parse(localStorage.getItem('cet-chat-settings') || '{}')
            if (!settings.apiKey) {
              return <p className="text-amber-600 text-xs">请先设置 API Key</p>
            }
            return null
          })()}
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4">
        ...
        {messages.map((msg) => (
          ...
          >
            ...
            />
            <div
              className={`max-w-[78%] sm:max-w-[70%] rounded-2xl px-3 py-2 sm:px-4 sm:py-2.5 text-sm leading-relaxed break-words ${
                msg.role === 'user'
        ...
        {loading && (
          <div className="flex items-start gap-2 sm:gap-3">
            <Avatar src={aiAvatar} name={partnerName} size={30} />
      ...
      <div className="border-t border-gray-200 p-3 sm:p-4 bg-white">
        <div className="flex gap-2 sm:gap-3 max-w-3xl mx-auto">
          <input
            ...
            placeholder="输入你的消息..."
            disabled={loading}
            className="flex-1 border border-gray-300 rounded-lg px-3 py-2 sm:px-4 sm:py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-400 disabled:opacity-50"
          />
          <button
            onClick={handleSend}
